defmodule ImgDecodeTest do
  ...
  test "decode png from file" do
    {:ok, img, shape, type, channels} = ImageRs.from_file(Path.join(__DIR__, "test.png"))
    assert type == :u8
    assert shape == {2, 3, 4}
  ...
  test "decode jpg from file" do
    {:ok, img, shape, type, channels} = ImageRs.from_file(Path.join(__DIR__, "test.jpg"))
    assert type == :u8
    assert shape == {2, 3, 3}
    assert channels == :rgb

    assert img ==
             <<180, 128, 70, 148, 128, 78, 89, 134, 101, 223, 171, 113, 182, 162, 112, 112, 157,
               124>>
  end

  test "decode png from memory" do
    {:ok, buffer} = File.read(Path.join(__DIR__, "test.png"))
    {:ok, img, shape, type, channels} = ImageRs.from_memory(buffer)
    assert type == :u8
    assert shape == {2, 3, 4}
    assert channels == :rgba
